...
class RFDecode:
    ...
    def __init__(
        ...
        # The PLL discriminator (--fm_pll) is a loop whose gains are tuned
        # against the input rate, so it is not a drop-in on the half-rate
        # analytic signal; keep it at the full rate it was calibrated for
        # rather than silently changing what it does.
        if extra_options.get("fm_pll", False):
            full_rate_demod = True
    ...
    def demodblock(self, data=None, mtf_level=0, fftdata=None, cut=False,
        ...
        # fm_pll forces full_rate_demod (see __init__), so
        # freq_hz_demod is freq_hz here and the PLL sees the
        # full-rate signal it was tuned against.
        if self.fm_pll:
            demod = unwrap_hilbert_pll(
                hilbert, self.freq_hz_demod, self.DecoderParams["ire0"],
