...
def _decode_field_worker(seq, start, raw_span, span_begin, mtf_level,
    ...
    try:
        ...
        # EFM, echo cancellation, fm_pll); otherwise the per-block CPU path
        # runs, and that is the one that gets the cross-job block reuse.
        gd = _worker_gpu()
        if gd is not None:
            ...
            for b, rawinput in zip(brange, raws):
