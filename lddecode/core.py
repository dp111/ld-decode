...
from .utils import findpeaks, findpulses, calczc, inrange, roundfloat, refine_pilot_zcs, refine_hsync_zcs, compute_linelocs_kernel
...
class RFDecode:
    ...
    def __init__(
        ...
        extra_options -- Dictionary of additional options (typically boolean) - these include:
          - PAL_V4300D_NotchFilter - cut 8.5mhz spurious signal (bin zeroing)
        ...
        # (manual, fixed), or True to auto-detect from the RF cepstrum.  The auto
        # path keeps a magnitude EMA, masks narrow spurs (e.g. the LD-V4300D
        # ~8.5 MHz tone) before the cepstrum, refines the tap amplitudes, picks
        # echoes as peaks standing above the local cepstral baseline (so the
        # smooth de-emphasis/band-pass envelope is not mistaken for an echo),
        # re-estimates continuously (tracking drift with disc radius) and only
        # applies the correction when it measurably reduces the echo - see
        # _echo_update()/_echo_reestimate().  Each worker thread owns its own
        # RFDecode (hence its own echo state), so no locking is needed.
    ...
    def v4300d_coherent_subtract(self, indata_fft, maxlines=10):
        ...
        lines (line-rate spacing) in this window, and those measure up to
        ~27x the window's median power on the test captures, so a new line
        is only accepted at >40x median - which a lone player spur at
        visible levels easily exceeds (-25 dBc measures ~60x).  Follow-up
        iterations clean up the residual of an imperfect fit, and are only
        allowed within +-30 kHz of an already-confirmed line (at a relaxed
        >5x gate), so real video comb lines are never subtracted.  maxlines
    ...
    def computevideofilters(self):
        ...
        # demodblock_cpu, only when the carriers are actually present (PAL
        # discs carry either analog audio or EFM, never both).  EFM and the
        # audio decode paths tap indata_fft directly and are unaffected.
        if SP["analog_audio"]:
            cut_left = sps.butter(
                DP["audio_notchorder"],
    ...
    def demodblock_cpu(self, data=None, mtf_level=0, fftdata=None, cut=False):
        ...
        if self.system == "PAL" and self.PAL_V4300D_CoherentSubtract and v4300_on:
            ...
            # coherently and subtract them in the time domain.  ~6-10 dB more
...
class LDdecode:
    ...
    def checkMTF(self, field, pfield=None):
        ...
        if not self.autoMTF:
            # np.maximum, not np.max: np.max(x, 0) treats 0 as the axis
            # argument and returns x unchanged, so the level went negative
            # past frame 10000 and MTF**negative blurred the HF instead.
            self.mtf_level = np.maximum(1 - (self.frameNumber / 10000), 0)
        else:
        ...
        while done is False:
            ...
            if f and f.valid:
                ...
                if self.useAGC and f.isFirstField and f.sync_confidence > 80:
                    ...
                    # measured white should sit at 100 IRE under the current
                    # calibration (comparing it against itself made this 0, so
                    # a pure white-gain drift could never retrigger the AGC)
